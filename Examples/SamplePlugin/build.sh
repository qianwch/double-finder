#!/bin/bash
# Builds SamplePlugin.dfplugin into ./.dist. With --install, also copies it to
# ~/Library/Application Support/Double Finder/Plugins (Double Finder picks it
# up at the next launch, or via Settings ▸ Plugins ▸ Rescan).
set -euo pipefail
cd "$(dirname "$0")"
...
# HOST's copy (next to the executable / Contents/Frameworks), never against the
# copy SwiftPM built here — two copies of PluginKit in one process would give
# the protocols two identities. Strip every rpath the build baked in.
